...
Python al ser un lenguaje mas natural para el humano es mucho mas facil
...
```
Codigo de Scala en programacion estructural
```Scala
def getAverage(nums: List[Int]): Double =
  var sum = 0
  for num <- nums do
    sum += num
  sum.toDouble / nums.length
```
...
Este codigo es usando 
```shell

scala-cli repl

```
Este es ocupando programacion funcional en Scala
```Scala
def getAverage(nums: List[Int]): Double =
  if nums.isEmpty then Double.NaN
  else nums.sum.toDouble / nums.length
```
Este es ocupando programacion funcional en python
```Python
...
```
Este es ocupando programacion funcional en haskell
```Haskell
